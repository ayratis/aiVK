...
public class AttachmentsAdapterConversation extends RecyclerView.Adapter<AttachmentsAdapterConversation.ViewHolder> {

    private List<Attachments> attachments;
    private OnAttachmentClickListener onAttachmentClickListener;

    public AttachmentsAdapterConversation(List<Attachments> attachments, OnAttachmentClickListener onAttachmentClickListener) {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.universal_item_conversation, parent, false);
    ...
    private void setPhoto(ViewHolder holder, Attachments attachment) {
        ...
        if (size != null) {
            ...
            int finalWidth = holder.unImage.getContext().getResources().getDisplayMetrics().widthPixels * 3 / 5;
    ...
    private void setDoc(final ViewHolder holder, Attachments attachment) {
        ...
        } else if(attachment.getDoc().getType() == 5){
            holder.unImage.setVisibility(View.GONE);
            holder.unCenterIcon.setVisibility(View.GONE);
            holder.unIcon.setImageResource(R.drawable.bt_audio);
            holder.unTitleText.setText("Голосовое сообщение");
            holder.unTitleUnderText.setVisibility(View.GONE);
            holder.unIcon.setOnClickListener(view -> onAttachmentClickListener.onAttachmentClick(attachment));
        } else {
            holder.unImage.setVisibility(View.GONE);
    ...
    private void setSticker(ViewHolder holder, Attachments attachment) {
        holder.unImage.setVisibility(View.VISIBLE);
        holder.unImage.setScaleType(ImageView.ScaleType.FIT_CENTER);
    ...
    private void setWall(ViewHolder holder, Attachments attachment) {
        if(attachment.getWall().getAttachments() != null && !attachment.getWall().getAttachments().isEmpty()) {
